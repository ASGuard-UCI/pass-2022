...
"""
This module provides access to the simulation time and contains a py_trees
timeout behavior using the simulation time
"""
...
class GameTime(object):
    ...
    """
    This (static) class provides access to the simulation time.
    ...
    @staticmethod
    def on_svl_tick(timestamp, frame):
        """
        Callback receiving the simulation time
        Update time only when frame is more recent that last frame
        """
        if GameTime._current_game_time < timestamp:
            GameTime._current_game_time = timestamp
            GameTime._last_frame = frame
...
class SimulationTimeCondition(py_trees.behaviour.Behaviour):
    ...
    It uses the simulation time, not the system time which is used by
    the py_trees timer.
