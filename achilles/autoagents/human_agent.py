...
class KeyboardControl(object):
    ...
    def _json_to_control(self):
        ...
        for entry in self._records['records']:
            control = lgsvl.VehicleControl(throttle=entry['control']['throttle'],
                                           steering=entry['control']['steering'],
                                           braking=entry['control']['braking'],
                                           handbrake=entry['control']['handbrake'],
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_q:
                    self._control.reverse = False if self._control.reverse else True
        ...
        self._control.steering = round(self._steer_cache, 1)
        self._control.braking = 1.0 if keys[K_DOWN] or keys[K_s] else 0.0
        self._control.handbrake = keys[K_SPACE]
    ...
    def _record_control(self):
        new_record = {
            'control': {
                ...
                'braking': self._control.braking,
                'handbrake': self._control.handbrake,
