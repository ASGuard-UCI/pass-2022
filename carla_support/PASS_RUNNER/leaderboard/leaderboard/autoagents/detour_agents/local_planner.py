...
import carla
from leaderboard.autoagents.detour_agents.controller import VehiclePIDController
...
class LocalPlanner(object):
    ...
    def add_detour_wp(self, k=1):
        ...
        for ii in range(k):
            old_next_waypoint = self._waypoints_queue[0+ii][0]
            left_detour_wp = old_next_waypoint.get_left_lane()
            right_detour_wp = old_next_waypoint.get_right_lane()
            if left_detour_wp is not None:
                detour_waypoints = [left_detour_wp]
            elif right_detour_wp is not None:
                detour_waypoints = [right_detour_wp]
            else:
                detour_waypoints = [self._map.get_waypoint(
                    old_next_waypoint.transform.location - 3 * old_next_waypoint.transform.get_right_vector())]

            # detour_waypoints = list(old_next_waypoint.next(self._sampling_radius))

            if len(detour_waypoints) == 0:
                break
            elif len(detour_waypoints) == 1:
                # only one option available ==> lanefollowing
                next_detour_waypoint = detour_waypoints[0]
                road_option = RoadOption.LANEFOLLOW
            else:
                # random choice between the possible options
                road_options_list = _retrieve_options(
                    detour_waypoints, old_next_waypoint)
                road_option = random.choice(road_options_list)
                next_detour_waypoint = detour_waypoints[road_options_list.index(
                    road_option)]

            # self._waypoints_queue.append((next_detour_waypoint, road_option))
            wps.append((next_detour_waypoint, road_option))
        for _ in range(k):
            self._waypoints_queue.popleft()
        for wp in wps[::-1]:
            self._waypoints_queue.appendleft(wp)
